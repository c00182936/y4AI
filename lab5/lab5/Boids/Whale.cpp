...
Whale::Whale(float x, float y, int wHeight, int wWidth)
{
...
void Whale::applyForce(Pvector force)
{
	acceleration.addVector(force);
}

Pvector Whale::Separation(vector<Feesh> Whale)
{
	...
	for (int i = 0; i < Whale.size(); i++)
	{
		...
		float d = position.distance(Whale[i].position);
		// If this is a fellow boid and it's too close, move away from it
		if ((d > 0) && (d < desiredseparation))
		{
			Pvector diff(0, 0);
			diff = diff.subTwoVector(position, Whale[i].position);
			diff.normalize();
...
Pvector Whale::Alignment(vector<Feesh> Whale)
{
	...
	for (int i = 0; i < Whale.size(); i++)
	{
		...
		{
			sum.addVector(Whale[i].velocity);
			count++;
		}
	}
...
Pvector Whale::Cohesion(vector<Feesh> Whale)
{
	...
	for (int i = 0; i < Whale.size(); i++)
	{
		...
		{
			sum.addVector(Whale[i].position);
			count++;
		}
	}
	if (count > 0)
...
void Whale::flock(vector<Feesh> v)
{
