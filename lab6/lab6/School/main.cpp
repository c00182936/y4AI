...
//	string action = "flock";
	...
	for (int i = 0; i < fishNum; i++) //Number of boids is hardcoded for testing pusposes.
	{
		...
		sf::CircleShape gShape(boidsSize*2, 3);
		shape.setFillColor(sf::Color(173, 182, 198));
	...
	{	
		...
		// These if statements prevent boids from moving off the screen through warpping
		// If boid exits right boundary
		if (whaleShape.getPosition().x > window_width)
			whaleShape.setPosition(whaleShape.getPosition().x - window_width, whaleShape.getPosition().y);
		// If boid exits bottom boundary
		if (whaleShape.getPosition().y > window_height)
			whaleShape.setPosition(whaleShape.getPosition().x, whaleShape.getPosition().y - window_height);
		// If boid exits left boundary
		if (whaleShape.getPosition().x < 0)
			whaleShape.setPosition(whaleShape.getPosition().x + window_width, whaleShape.getPosition().y);
		// If boid exits top boundary
		if (whaleShape.getPosition().y < 0)
			whaleShape.setPosition(whaleShape.getPosition().x, whaleShape.getPosition().y + window_height);
