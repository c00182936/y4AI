...
Gull::Gull(float x, float y, int wHeight, int wWidth)
{
...
void Gull::applyForce(Pvector force)
{
	acceleration.addVector(force);
}

Pvector Gull::Separation(vector<Gull> gull)
{
	...
	for (int i = 0; i < gull.size(); i++)
	{
		// Calculate distance from current boid to boid we're looking at
		float d = position.distance(gull[i].position);
		...
		{
			Pvector diff(0, 0);
			diff = diff.subTwoVector(position, gull[i].position);
			diff.normalize();
	...
	for (size_t i = 0; i < fishVec.size(); i++)
	{
		float d = position.distance(fishVec[i].position);
		//float whaleDist = position.distance(whalePos);
		//if ((whaleDist > 0) && (whaleDist < desiredseparation + 10))
		//{
		//	Pvector pred(0, 0);
...
Pvector Gull::Alignment(vector<Gull> gull)
{
...
Pvector Gull::Cohesion(vector<Gull> gull)
{
